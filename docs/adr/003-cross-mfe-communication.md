...
1. **Routing** — el Shell pasa `pokemonId`/`name` a MF1 vía **route param** (`/pokemon/:name`).
   ...
   >
   > **Actualización (revisión visual post Fase 5):** se agregó navegación prev/next dentro del detalle (`docs/adr/013`). Mismo patrón: MF1 no navega solo, expone `onNavigate?: (name: string) => void`; el Shell se lo pasa (`navigate(`/pokemon/${nextName}`)`) y MF1 simplemente lo invoca con `id - 1` / `id + 1`. En modo standalone (`apps/mf1-detail/src/App.tsx`) `onNavigate` apunta a un `useState` local, así prev/next también funciona abriendo `:3001` directo.
