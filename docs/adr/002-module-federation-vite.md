...
- Shell consume ambos remotes vía `import('mf1Detail/PokemonDetail')` dentro de las rutas correspondientes, con un `ErrorBoundary` alrededor de cada uno (`RemoteBoundary.tsx`). Originalmente ese `import()` estaba envuelto en `React.lazy` + `Suspense` — dejó de estarlo, ver el bug de producción abajo.
...
### Toda la UI del Shell se veía "como mobile" en desktop (después del fix anterior)

> **Actualización (mismo ciclo de verificación post-deploy):** con el bug anterior ya resuelto, el usuario reportó que en el Shell (Home, y en general) los elementos que deberían verse solo en desktop no aparecían y viceversa — el header completo (nav, buscador, menú de usuario) quedaba oculto y la barra flotante de mobile (`MobileBottomNav`) se mostraba siempre, sin importar el ancho de la ventana. Otra vez: invisible en `pnpm dev`.
>
> **Descartado primero, por las dudas** (con capturas y `window.matchMedia`): no era el viewport real (`window.innerWidth` y `matchMedia('(min-width: 640px)')` reportaban correctamente `1440`/`true`), no era caché del navegador (confirmado limpiando caché vía CDP y con `Network.clearBrowserCache` antes de cada prueba), y no era el minificador de CSS de Vite (se probó `cssMinify: false` — con eso *solo*, sin ningún otro cambio, el bug seguía igual apenas se cargaba el remote; solo "parecía" arreglado en una prueba anterior porque ese test específico no tenía a MF1 corriendo).
>
> **Causa raíz real, confirmada inspeccionando el cascade CSS directamente** (vía CDP, recorriendo `document.styleSheets` con soporte para `@layer`/`@media`/`@supports` anidados — no alcanza con mirar la regla "obvia", hay que ver *cuál gana*): Shell y cada remote compilan su **propio** CSS de Tailwind v4 de forma independiente, cada uno con su propio `@layer utilities { ... }`. Cuando ambos CSS terminan en el mismo documento (Shell vía `<link>` propio + el CSS del remote inyectado por `bundleAllCSS: true`), los navegadores **fusionan capas del mismo nombre** (`utilities` de Shell y `utilities` del remote pasan a ser una sola capa combinada) — y dentro de esa capa fusionada, el orden pasa a ser el orden de aparición combinado de *ambas* hojas. El resultado: la copia (duplicada, no condicional) de `.flex{display:flex}` que cada remote también genera para sus propios componentes termina, en el documento combinado, *después* de la regla `.sm\:hidden{display:none}` de Shell — y como ambas tienen exactamente la misma especificidad (un solo selector de clase), gana la que aparece último. La intención de la responsividad de Shell queda enterrada por una clase base sin relación alguna, de un bundle completamente distinto.
>
> **Fix real:** se puso `bundleAllCSS: false` en MF1 y MF2 (estaba en `true` desde el bug del CSS de arriba). Ya no hace falta: el `@source '../../mf1-detail/src'` / `@source '../../mf2-history/src'` en `apps/shell/src/index.css` (agregado en aquel primer fix) hace que el build de Tailwind del **Shell** ya sea un superset estricto de todas las clases que usan los remotes — así que nunca necesitó depender de que el CSS de los remotes llegara al documento. Al dejar de inyectar ese segundo CSS por completo, desaparece la fusión de capas y con ella la colisión. Verificado: en producción real (`vite preview` en puertos separados, igual que Vercel) el header y la barra mobile alternan correctamente según el viewport, en detalle/historial/home, y el modo standalone de cada MF (su propio `index.html`, que no pasa por este mecanismo en absoluto) sigue viéndose idéntico a como se veía antes.
>
> **Por qué el primer fix (`bundleAllCSS: true`) no era el correcto ya desde un principio:** ese fix resolvía un problema real (el CSS del remote no llegaba al Shell en dev), pero el propio `@source` agregado en el mismo cambio ya dejaba obsoleta la necesidad de `bundleAllCSS` — los dos fixes atacaban el mismo síntoma por dos caminos distintos, y quedarse con ambos activos es lo que, meses de iteración después, terminó rompiendo la responsividad completa del Shell en producción. Un solo mecanismo (el `@source`, que no depende de que nada se cargue en runtime) alcanzaba.
>
> **Lección:** cargar dos bundles de CSS de Tailwind independientes en el mismo documento no es seguro en general, incluso si ninguno de los dos tiene errores — las reglas CSS de un microfrontend pueden pisar silenciosamente las de otro con solo compartir un nombre de clase utilitaria común (`flex`, `hidden`, cualquier utilidad base), sin que haga falta que las apps compartan ningún componente ni intención. Si un remote no *necesita* enviar su propio CSS al host (porque el host ya cubre sus clases, como acá), mejor no enviarlo.
