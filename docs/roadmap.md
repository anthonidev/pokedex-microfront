...
**Bug conocido, aún no corregido:** `GET /pokemon-species/{name}` devuelve 404 para variantes de forma (ej. `pikachu-libre`, cualquier mega-evolución/forma regional) — ese endpoint usa el nombre de la especie base, no el de la variante. Afecta la descripción (flavor text) del detalle. Fix: usar `pokemon.species.name` (viene en la respuesta de `/pokemon/{name}`) en vez del `name` tipeado/de ruta. Sigue pendiente porque no se tocó la lógica de datos en la retomada de esta vista (ver entrada siguiente), solo layout — queda para cuando el usuario pida específicamente tocar esa parte.
