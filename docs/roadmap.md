...
- [x] Opcional: deploy demo. Se decidió Vercel (3 apps son builds estáticos, sin backend propio — Railway/Docker no aportarían nada acá). Repo preparado para el deploy: URLs de los remotes de Module Federation parametrizadas por variable de entorno (`VITE_MF1_ENTRY_URL`/`VITE_MF2_ENTRY_URL` en `apps/shell/vite.config.ts`, con fallback a `localhost` para que `pnpm dev` no necesite nada especial), `vercel.json` por app (rewrite SPA en Shell, CORS en los remotes para que Module Federation pueda cargar su `remoteEntry.js` cross-origin). Pasos del dashboard de Vercel documentados en el README raíz. El usuario creó los 3 proyectos y deployó — ver el bug real que surgió recién ahí, abajo.

### Bug crítico post-deploy: el detalle y el historial no cargaban en producción

Con las 3 apps ya en Vercel y las env vars bien puestas, `/pokemon/:name` y `/history` quedaban en blanco (mostrando el skeleton para siempre) — sin ningún error en consola. **Invisible en `pnpm dev` durante toda la sesión** porque ahí Shell/MF1/MF2 comparten el mismo servidor Vite; solo se manifiesta cruzando orígenes reales (producción, o `vite preview` en puertos separados simulando lo mismo localmente — así se reprodujo y depuró, sin gastar más ciclos de deploy).

- [x] **Causa raíz**: el módulo remoto se resolvía perfecto a nivel del runtime de Module Federation (verificado inspeccionando `window.__mf_module_cache__` directamente — el componente real estaba ahí), pero `React.lazy()` + `<Suspense>` nunca se enteraban de esa resolución y quedaban suspendidos para siempre (fiber del boundary atascado en `SuspenseComponent`, sin error, sin reintento). **No era el clásico problema de "dos copias de React"** — se descartó explícitamente probando `singleton: true` (ver ADR 002).
- [x] **Fix**: `RemoteBoundary.tsx` — reemplazado `lazy`/`Suspense` por un `useEffect` manual que resuelve la misma promesa vía `.then()`/`useState`, sin depender del protocolo interno de "lanzar la promesa durante el render" que no se llevaba bien con este runtime en producción cross-origin.
- [x] **Efecto colateral corregido en el mismo cambio**: el botón "Reintentar" del error boundary no funcionaba de forma confiable (el runtime cachea un fallo de carga de `remoteEntry.js` a nivel de sesión) — ahora hace `window.location.reload()` en vez de reintentar en memoria.
- Detalle completo, con todos los pasos de diagnóstico, en el ADR 002.

### Segundo bug crítico post-deploy: toda la UI del Shell se veía "como mobile" en desktop

Con el bug anterior ya resuelto, apareció uno nuevo: en el Shell, los elementos que debían mostrarse solo en desktop (header completo) quedaban ocultos, y la barra flotante de mobile se mostraba siempre — sin importar el ancho real de la ventana. Otra vez invisible en `pnpm dev`.

- [x] **Causa raíz**: dos hojas de CSS de Tailwind v4 compiladas de forma independiente (Shell + cada remote) terminan en el mismo documento (el CSS del remote se inyectaba vía `bundleAllCSS: true`). Los navegadores **fusionan capas `@layer` del mismo nombre** entre hojas distintas — la copia duplicada de `.flex{display:flex}` que cada remote también genera para sus propios componentes terminaba, en el documento combinado, *después* de `.sm\:hidden{display:none}` de Shell, y con igual especificidad gana la última. Verificado con inspección directa del cascade vía CDP, no por prueba y error — se descartaron primero (con evidencia) el viewport, la caché del navegador y la minificación de CSS antes de encontrar la causa real.
- [x] **Fix**: `bundleAllCSS: false` en MF1 y MF2. El `@source '../../mf1-detail/src'`/`@source '../../mf2-history/src'` que ya tiene `apps/shell/src/index.css` (del primer bug del CSS, ver ADR 002) hace que el Shell ya sea un superset estricto de las clases de ambos remotes — nunca hizo falta que el CSS de los remotes llegara al documento. Verificado en producción real (`vite preview` en puertos separados) en Home/Detalle/Historial, y confirmado que el modo standalone de cada MF no se vio afectado.
- Detalle completo, con todos los pasos de diagnóstico y por qué el primer fix del CSS (ADR 002) ya había dejado esto obsoleto sin que nadie lo notara, en el ADR 002.
